...
public class TSelection implements CommandExecutor {

    private final Towny_request plugin;

    public TSelection(Towny_request plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String s, @NotNull String[] args) {
        ...
        // Переменные:
        String messagePermission = plugin.getConfig().getString("broadcasts.t-selection.no-permission");
        messagePermission = messagePermission.replace("&", "\u00a7");
        messagePermission = messagePermission.replace("{PLAYER}", sender.getName());

        String messageTSelection = plugin.getConfig().getString("broadcasts.t-selection.message");
        messageTSelection = messageTSelection.replace("&", "\u00a7");

        String buttonMessage = plugin.getConfig().getString("broadcasts.t-selection.button-message");
        buttonMessage = buttonMessage.replace("&", "\u00a7");

        String buttonPrompt = plugin.getConfig().getString("broadcasts.t-selection.button-prompt");
        buttonPrompt = buttonPrompt.replace("&", "\u00a7");

        String successMessage = plugin.getConfig().getString("broadcasts.t-selection.message-successfully");
        successMessage = successMessage.replace("&", "\u00a7");

        String noTown = plugin.getConfig().getString("broadcasts.t-selection.no-town");
        successMessage = successMessage.replace("&", "\u00a7");

        String soundTAskValue = plugin.getConfig().getString("broadcasts.t-selection.sound-t-selection");
        ...
        if (!sender.hasPermission("townyrequest.t.selection")) {
        ...
        messageTSelection = messageTSelection.replace("{TOWN}", town.getName());
        ...
        Bukkit.broadcastMessage(messageTSelection);
